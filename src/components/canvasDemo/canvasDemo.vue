<template>
  <div>
    ...
    <div
      style="background-color:rgba(43, 189, 226, 0.877);width: 1280px;height: 860px; position: relative;"
    >
      <canvas
        id="canvas"
...
export default {
  ...
  methods: {
    ...
    drawImage() {
      ...
      var drawLeft =
        ((this.leftDis/this.imgScale - this.imgX) / (this.image2.width )) *
        this.canvas.width; //计算当前分割线相对于image的比例，并计算出相对于画布的宽度。
      this.cs.drawImage(
        this.image2,
        0, //sx开始裁剪的位置
        0, //sy
        this.leftDis, //swidth裁剪的宽度
        this.image2.height, //sheight裁剪的高度
        this.imgX, //x画布上的x坐标
        this.imgY, //Y
        this.leftDis, //width要使用的图像宽度
        // image2.width * imgScale,
        this.image2.height //height要使用的图像高度
      );
    },
    //切换对比方向
    changeDirection() {
